...
class GeneticAlgorithm:
    ...
    def mutation(chromosome):
        ...
        if random.random() < config['mutation_probability'] / 3:
            chromosome.kp = chromosome.kp + random.random()/config['max_gain_value']
        if chromosome.kp < 0:
            chromosome.kp = random.random() * config['max_gain_value']

        elif random.random() < config['mutation_probability'] * 2/3:
            chromosome.ki = chromosome.ki + random.random()/config['max_gain_value']
        if chromosome.ki < 0:
            chromosome.ki = random.random() * config['max_gain_value']

        elif random.random() < config['mutation_probability']:
            chromosome.kd = chromosome.kd + random.random()/config['max_gain_value']
        if chromosome.kd < 0:
            chromosome.kd = random.random() * config['max_gain_value']
